...
namespace NzbDrone.Core.Test.ParserTests.NewParser
{
    [TestFixture]
    [Category("ParserTest")]
    public class QualityParserFixture : CoreTest<NewParseProvider>
    {
        ...
        public void parsing_our_own_quality_enum_name(Quality quality)
        {
            ...
            var result = Subject.ParseTitle(fileName);
        ...
        {
            var result = Subject.ParseTitle(title);
            result.Quality.Quality.Should().Be(quality);

            var version = proper ? 2 : 1;
            result.Quality.Revision.Version.Should().Be(version);
        }
    }
}
